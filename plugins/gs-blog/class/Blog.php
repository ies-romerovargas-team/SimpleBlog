...
class Blog 
{
	...
	* @return bool
	*/  
	public function saveRSS($new_rss)
	{
	...
	*/  
    ...
    if(!count($posts) < 1) {
      foreach ($posts as $post) 
      {
        ...
        $RSSString .= "\t<pubDate>".date(DATE_RSS,strtotime($RSSDate))."</pubDate>\n";
	...
	*/  
    ...
    if(!count($posts) < 1) {
      ...
      return XMLsave($xml, BLOGCACHEFILE);
    }
	}
	...
	*/  
	...
	{
		preg_match('/{\$\s*([a-zA-Z0-9_]+)(\s+[^\$]+)?\s*\$}/', $content, $matches);
		if(isset($matches[0]))
		{
